...
(() => {
  ...
  // Cap the longest side generously so uploads stay crisp on large tiles
  // and retina displays; WebP keeps the sidecar reasonable even at this size.
  const MAX_DIM = 2400;
  ...
  const stylesheet =
    ...
    // Once filled, drop the grey placeholder tint so a contain letterbox shows
    // the host element's own background (set by the author) instead of grey.
    ':host([data-filled]) .frame{background:transparent}' +
    // .frame img (clipped) and .spill (unclipped ghost + handles) share the
    // same left/top/width/height in frame-%, computed by _applyView(), so the
    // inside-mask crop and the outside-mask spill stay pixel-aligned.
    // touch-action stays auto so a finger landing on the image still scrolls
    // the page on mobile; it's locked to none only while reframing (below).
    ...
    // An author-provided src= means the slot ships filled — hide the dashed
    // drop ring immediately on parse so it never flashes over the real image
    // before the JS fill cycle marks the slot [data-filled].
    ':host([src]) .ring{display:none}' +
    ':host([src]) .empty{display:none}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      load();
      this._render();
      // A host framework (e.g. the DC runtime / React) may append the
      // element and only THEN assign a large src= attribute, sometimes
      // without re-triggering a fill — leaving the slot in its empty
      // drop-zone state with the dashed ring over a loaded image. Re-render
      // a few times after connect so a late src reliably flips the slot to
      // filled regardless of when the host sets it.
      requestAnimationFrame(() => { if (this.isConnected) this._render(); });
      this._lateFill = [120, 400, 900].map((ms) =>
